        ...
        *configTimeText = '\0';
        ...
        while (f_gets(buffer, 64, &cfgFile))
        {
            ...
            else if (strstr(buffer, "derived_channels="))
            {
                ...
                stored_config_temp.rawBytes[NV_DERIVED_CHANNELS_1] = (derived_channels_val >> 8) & 0xff;
                stored_config_temp.rawBytes[NV_DERIVED_CHANNELS_2] = (derived_channels_val >> 16) & 0xff;
                stored_config_temp.rawBytes[NV_DERIVED_CHANNELS_3] = (derived_channels_val >> 24) & 0xff;
                stored_config_temp.rawBytes[NV_DERIVED_CHANNELS_4] = (derived_channels_val >> 32) & 0xff;
                stored_config_temp.rawBytes[NV_DERIVED_CHANNELS_5] = (derived_channels_val >> 40) & 0xff;
                stored_config_temp.rawBytes[NV_DERIVED_CHANNELS_6] = (derived_channels_val >> 48) & 0xff;
                stored_config_temp.rawBytes[NV_DERIVED_CHANNELS_7] = (derived_channels_val >> 56) & 0xff;
            }
        }
        file_status = f_close(&cfgFile);
        HAL_Delay(50);  // 50ms
        ...
        if (stored_config_temp.chEnExg1_24Bit
                || stored_config_temp.chEnExg2_24Bit
                || stored_config_temp.chEnExg1_16Bit
                || stored_config_temp.chEnExg2_16Bit)
        {
          stored_config_temp.chEnIntADC3 = 0;
          stored_config_temp.chEnIntADC2 = 0;
            triggerSdCardUpdate = 1;
        ...
        memcpy(&storedConfig->rawBytes[NV_DERIVED_CHANNELS_3], &stored_config_temp.rawBytes[NV_DERIVED_CHANNELS_3], 5);
        /* Infomem C - Bytes 128-132 - MPL related settings - no longer used/supported */
        memcpy(&storedConfig->rawBytes[NV_SENSORS3], &stored_config_temp.rawBytes[NV_SENSORS3], 5);
        ...
        memcpy(&storedConfig->rawBytes[NV_SD_SHIMMER_NAME], &stored_config_temp.rawBytes[NV_SD_SHIMMER_NAME], 37);

        /* Infomem B - Bytes 256-381 - Center and Node MAC addresses */
        memcpy(&storedConfig->rawBytes[NV_CENTER], &stored_config_temp.rawBytes[NV_CENTER], NV_NUM_BYTES_SYNC_CENTER_NODE_ADDRS);
