...
/*   case SET_LSM303DLHC_ACCEL_SAMPLING_RATE_COMMAND:
    if(btArgs[0] < 10)
      storedConfig[NV_CONFIG_SETUP_BYTE0] = (storedConfig[NV_CONFIG_SETUP_BYTE0]&0x0F) + ((btArgs[0]&0x0F)<<4);
    else
    ...
    if(btArgs[0]>0 && btArgs[0]<8)
      storedConfig[NV_CONFIG_SETUP_BYTE2] = (storedConfig[NV_CONFIG_SETUP_BYTE2]&0x1F) + ((btArgs[0]&0x07)<<5);
    else
    ...
    case SET_LSM303DLHC_MAG_SAMPLING_RATE_COMMAND:
    if(btArgs[0] < 8)
      storedConfig[NV_CONFIG_SETUP_BYTE2] = (storedConfig[NV_CONFIG_SETUP_BYTE2]&0xE3) + ((btArgs[0]&0x07)<<2);
    else
    ...
    if(btArgs[0] < 4)
      storedConfig[NV_CONFIG_SETUP_BYTE2] = (storedConfig[NV_CONFIG_SETUP_BYTE2]&0xFC) + (btArgs[0]&0x03);
    else
    ...
    if(btArgs[0] < 4)
      storedConfig[NV_CONFIG_SETUP_BYTE3] = (storedConfig[NV_CONFIG_SETUP_BYTE3]&0x3F) + ((btArgs[0]&0x03)<<6);
    else
    ...
    if(btArgs[0] < 4)
      storedConfig[NV_CONFIG_SETUP_BYTE3] = (storedConfig[NV_CONFIG_SETUP_BYTE3]&0xCF) + ((btArgs[0]&0x03)<<4);
    else
    ...
    case SET_INTERNAL_EXP_POWER_ENABLE_COMMAND:
    if(btArgs[0] == 1)
      storedConfig[NV_CONFIG_SETUP_BYTE3] = (storedConfig[NV_CONFIG_SETUP_BYTE3]&0xFE) + (btArgs[0]&0x01);
    else
...
//         uniqueSerialResponse = 0;
         ...
         *(bt_tx_data + packet_length++) = exgLength;
         if(exgLength) {
            if(exgChip)
               memcpy((bt_tx_data+packet_length), &storedConfig->exgADS1292rRegsCh2.rawBytes[0], exgLength);
            else
               memcpy((bt_tx_data+packet_length), &storedConfig->exgADS1292rRegsCh1.rawBytes[0], exgLength);
         ...
         *(bt_tx_data + packet_length++) = storedConfig->rawBytes[NV_DERIVED_CHANNELS_0];
         *(bt_tx_data + packet_length++) = storedConfig->rawBytes[NV_DERIVED_CHANNELS_1];
         *(bt_tx_data + packet_length++) = storedConfig->rawBytes[NV_DERIVED_CHANNELS_2];
         *(bt_tx_data + packet_length++) = storedConfig->rawBytes[NV_DERIVED_CHANNELS_3];
         *(bt_tx_data + packet_length++) = storedConfig->rawBytes[NV_DERIVED_CHANNELS_4];
         *(bt_tx_data + packet_length++) = storedConfig->rawBytes[NV_DERIVED_CHANNELS_5];
         *(bt_tx_data + packet_length++) = storedConfig->rawBytes[NV_DERIVED_CHANNELS_6];
         *(bt_tx_data + packet_length++) = storedConfig->rawBytes[NV_DERIVED_CHANNELS_7];
         derivedChannelResponse = 0;
      } else if(rwcResponse){
