...
void ms_clock_interrupt(void)
{
  ...
  {
    __bic_SR_register_on_exit(LPM3_bits + GIE); //exit LPM3 and enable interrupts
  }
  else if (in_delay_ms) //must NOT be in main loop
  {
    __bic_SR_register_on_exit(LPM0_bits + GIE); //exit LPM0 and enable interrupts
  }
#endif
}

int ms_clock_enable(void)
{
  if (clock.enabled)
  {
    return 0;
...
int ms_clock_get_aclk_freq(unsigned long *aclk)
{
  aclk[0] = clock.aclk;
  return 0;
}

int ms_clock_get_clock_ms(unsigned long *count)
{
  if (!count)
  {
    return 1;
...
#if defined(SHIMMER3)
int ms_clock_delay_ms(unsigned long num_ms)
{
...
int ms_clock_register_timer_cb(void (*timer_cb)(void), unsigned long num_ms, uint8_t exit_lpm)
{
  if (!timer_cb || !num_ms)
  {
