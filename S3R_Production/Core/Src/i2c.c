...
void I2C1_MemRxCpltCallback(I2C_HandleTypeDef *hi2c)
{
  ...
  {
  case I2C_LIS2MDL_MAG:
    memcpy(sensing.packetBuffers[sensing.dataWrBufIdx].dataBuf + sensing.ptr.mag1,
...
void BMP180TxDoneHandler(void)
{
  ...
  else if (i2c1Sens.status == I2C_STAT_BMP180_TEMP_GET_T)
  {
    ...
    BMP180_tempReadRx(sensing.packetBuffers[sensing.dataWrBufIdx].dataBuf
  ...
  else if (i2c1Sens.status == I2C_STAT_BMP180_PRES_GET_T)
  {
    ...
    BMP180_presReadRx(
        sensing.packetBuffers[sensing.dataWrBufIdx].dataBuf + sensing.ptr.pressure);
  }
  else
  {
    i2c1Sens.status = I2C_STAT_IDLE;
...
void BMP280RxDoneHandler(void)
{
  ...
  {
    BMP280_tempReadDone(sensing.packetBuffers[sensing.dataWrBufIdx].dataBuf
        + sensing.ptr.temperature);
...
{
  memcpy(sensing.packetBuffers[sensing.dataWrBufIdx].dataBuf + sensing.ptr.gyro,
...
{
  memcpy(sensing.packetBuffers[sensing.dataWrBufIdx].dataBuf + sensing.ptr.accel3,
...
void MPU9250MagTxDoneHandler(void)
{
  ...
  //  //MPU9250_magReadDone( sensing.packetBuffers[sensing.dataWrBufIdx].dataBuf
  //  + sensing.ptr.mpu9250Mag); MPU9250_magReadDone(i2cSens_buf.mpu9250MagBuf);
  //} else {
...
{
  memcpy(sensing.packetBuffers[sensing.dataWrBufIdx].dataBuf + sensing.ptr.mag2,
...
{
  memcpy(sensing.packetBuffers[sensing.dataWrBufIdx].dataBuf + sensing.ptr.accel2,
...
{
  //static uint8_t i=0;
  memcpy(sensing.packetBuffers[sensing.dataWrBufIdx].dataBuf + sensing.ptr.stc3100Batt,
