...
usbx_cdc_acm_t usbx_cdc_tx_rx = {NULL,cdc_rx_buffer,0,APP_RX_DATA_SIZE,0,0,UX_STATE_RESET,UX_STATE_RESET,usbx_success,usbx_success,0,1,0,1,0,0,NULL,0,0};
/* USER CODE END PTD */
...
{
  if (usbx_cdc_tx_rx.tx_active || usbx_cdc_tx_rx.tx_pending)
  {
          usbx_cdc_tx_rx.tx_result = usbx_busy;
          return usbx_busy;
  }
    ...
    case UX_STATE_WAIT:
      ...
      else if (status == UX_STATE_NEXT)
      {
       ...
       if(!usbx_cdc_tx_rx.rx_command_ready)
       {
         ...
         usbx_cdc_tx_rx.rx_command_ready = 1;
       }

      }
      else if (status == UX_STATE_WAIT)
      {
        // Partial reception: update count and mark pending
...
usbx_cdc_acm_result_t USBX_CDC_ACM_Receive(uint8_t* buffer, uint16_t size)
{
    ...
    usbx_cdc_tx_rx.rx_command_ready = 0;
    return usbx_success;
}
/* USER CODE END 1 */
