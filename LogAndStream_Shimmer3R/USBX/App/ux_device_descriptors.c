...
/* Bounded copy of the EEPROM brand manufacturer string (16 chars + NUL) */
static char runtime_manufacturer_string[EEPROM_BRAND_USB_MAX_CHARS + 1];
...
#pragma data_alignment = 4
#endif /* defined ( __ICCARM__ ) */
__ALIGN_BEGIN UCHAR USBD_language_id_framework[LANGUAGE_ID_MAX_LENGTH] __ALIGN_END
    = { 0 };
...
static void
USBD_FrameWork_CDCDesc(USBD_DevClassHandleTypeDef *pdev, uint32_t pConf, uint32_t *Sze);
...
uint8_t *USBD_Get_Device_Framework_Speed(uint8_t Speed, ULONG *Length)
{
  ...
  if (USBD_FULL_SPEED == Speed)
  {
    USBD_Device_Framework_Builder(
        &USBD_Device_FS, pDevFrameWorkDesc_FS, UserClassInstance, Speed);
  ...
  {
    USBD_Device_Framework_Builder(
        &USBD_Device_HS, pDevFrameWorkDesc_HS, UserClassInstance, Speed);
...
uint8_t *USBD_Get_String_Framework(ULONG *Length)
{
   ...
   * ShimEeprom_getBrandUsb() already returns a validated, NUL-terminated
   * string of at most EEPROM_BRAND_USB_MAX_CHARS, but copy through a bounded
   * local anyway so USBD_Desc_GetString()'s unbounded NUL-walk can never run
   * past a malformed source, whatever that accessor may do in future. */
  const char *manufacturerStr = USBD_MANUFACTURER_STRING;
  if (ShimEeprom_isBrandCustomer())
  {
    strncpy(runtime_manufacturer_string, ShimEeprom_getBrandUsb(),
        sizeof(runtime_manufacturer_string) - 1U);
    runtime_manufacturer_string[sizeof(runtime_manufacturer_string) - 1U] = '\0';
    manufacturerStr = runtime_manufacturer_string;
  }
  USBD_Desc_GetString((uint8_t *) manufacturerStr, USBD_string_framework + count, &len);
  ...
  while (Idx_Instance < USBD_MAX_SUPPORTED_CLASS)
  {
        ...
        && (UserClassInstance[Idx_Instance] != CLASS_TYPE_NONE))
    {
      ...
      (void) USBD_FrameWork_AddClass(pdev,
          (USBD_CompositeClassTypeDef) UserClassInstance[Idx_Instance], 0,
          Speed, (pDevFrameWorkDesc + pdev->CurrDevDescSz));
...
static void USBD_FrameWork_MSCDesc(USBD_DevClassHandleTypeDef *pdev, uint32_t pConf, uint32_t *Sze)
{
  ...
  __USBD_FRAMEWORK_SET_IF((pdev->tclasslist[pdev->classId].Ifs[0]), (0U),
      (uint8_t) (pdev->tclasslist[pdev->classId].NumEps), (0x08U), (0x06U),
      (0x50U), (0U));
...
static void USBD_FrameWork_CDCDesc(USBD_DevClassHandleTypeDef *pdev, uint32_t pConf, uint32_t *Sze)
{
  ...
  *Sze += (uint32_t) sizeof(USBD_IadDescTypedef);
  ...
  __USBD_FRAMEWORK_SET_IF(
      pdev->tclasslist[pdev->classId].Ifs[0], 0U, 1U, 0x02, 0x02U, 0x01U, 0U);
  ...
  *Sze += (uint32_t) sizeof(USBD_CDCUnionFuncDescTypedef);
  ...
  __USBD_FRAMEWORK_SET_EP(pdev->tclasslist[pdev->classId].Eps[2].add,
      USBD_EP_TYPE_INTR, (uint16_t) pdev->tclasslist[pdev->classId].Eps[2].size,
  ...
  /* Append Endpoint descriptor to Configuration descriptor */
  __USBD_FRAMEWORK_SET_EP((pdev->tclasslist[pdev->classId].Eps[0].add), (USBD_EP_TYPE_BULK),
      (uint16_t) (pdev->tclasslist[pdev->classId].Eps[0].size), (0x00U), (0x00U));

  /* Append Endpoint descriptor to Configuration descriptor */
  __USBD_FRAMEWORK_SET_EP((pdev->tclasslist[pdev->classId].Eps[1].add), (USBD_EP_TYPE_BULK),
      (uint16_t) (pdev->tclasslist[pdev->classId].Eps[1].size), (0x00U), (0x00U));
